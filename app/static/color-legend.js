...
function Legend(color, svg, {
    ...
    if(!(color.interpolate) & !(color.invertExtent) & !(color.interpolator) & (color.domain().length > 10)){
        return;
    }
...
function Swatches(color, {
  ...
  if(!(color.interpolate) & !(color.invertExtent) & !(color.interpolator) & (color.domain().length >= 18)){
    return "";
  }
  ...
  <div>${domain.map(value => `<span class="${id}" style="--color: ${color(value)}">${format(value)}</span>`).reduce((a, b) => a + b, "")}</div>`;
  }
}