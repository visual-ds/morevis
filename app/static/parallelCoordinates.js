class ParallelCoordinates {
	...
	draw(){
		var data = this._vis.data.grouped
	    ...
	    let line = d3.line(),
	        axis = d3.axisLeft().tickFormat(d3.format(".3s")),
	        background,
	        foreground;
            ...
            .each(function(d) { 
            	...
            	d3.select(this).call(y[d].zoom = d3.zoom()
					...
					.on("zoom", function(event, id){
						...
						d3.select(this).call(d3.axisLeft(newY).tickFormat(d3.format(".3s")))
    ...
    restart(self){
    	...
    	for(let d of dimensions){
		    ...
		    selected_axis.call(d3.axisLeft().tickFormat(d3.format(".3s")).scale(y)); 
	...
	updateComponents(selected){
		var selected_objects = selected.map(d => d.object)
		this._vis.set_filtered_data(selected_objects); 
    }
}